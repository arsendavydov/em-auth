...
- ✅ Линтинг и типы: **Ruff** (lint + format) и **Pyright**; в **GitHub Actions** проверки выполняются **перед** сборкой образов и деплоем в k3s.

### Стек

- `Python 3.12`, `FastAPI`, `PostgreSQL 16`, `SQLAlchemy 2`, `Alembic`
- `PyJWT`, `bcrypt`
- `Ruff`, `Pyright` (конфиг `fastapi/pyproject.toml`, скрипт `fastapi/scripts/lint.sh`)
- `nginx` (reverse proxy, rate limiting)
- `Docker` / `docker compose` (local)
- `k3s` + `GitHub Actions` (production deploy; в workflow: unit-тесты + ruff + pyright → build → deploy)
